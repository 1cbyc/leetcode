    ...
    <article className="space-y-6">
      <section className="space-y-3">
        <p>
          ...
          in left half, count how many elements in right half satisfy nums[i] >
          2 * nums[j]. use two-pointer technique: as left element increases,
          right pointer can only move forward.
        </p>
        <p>
          during merge phase, before merging, count pairs: for each left[i],
          find how many right[j] satisfy left[i] > 2 * right[j]. then merge
      ...
      <section className="space-y-3">
        ...
        <ul className="list-disc space-y-2 pl-5">
          <li>for each left[i], find j where left[i] > 2 * right[j]</li>
